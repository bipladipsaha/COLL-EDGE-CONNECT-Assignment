...
import { FiPlus, FiSearch, FiFilter, FiCheckCircle, FiClock, FiAlertCircle, FiList, FiCheckSquare } from 'react-icons/fi';
...
const Dashboard = () => {
  ...
  const fetchTasks = useCallback(async () => {
    setIsLoading(true);
    try {
      // First get basic stats (all tasks)
      const statsRes = await api.get('/tasks');
      const allTasks = statsRes.data.data.tasks;
      setStats({
      ...
      // Then get paginated/filtered tasks
      ...
      const endpoint = search ? `/tasks/search?q=${search}&page=${page}&limit=10` : `/tasks${queryParams}`;
    ...
    <div className="max-w-7xl mx-auto pb-10">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">Here's what's going on with your tasks today.</p>
        </div>
        <button 
